...
def instructor_dashboard_2(request, course_id):
    ...
    try:
        ...
        log.error(
            u"Unable to find course with course key {course_key} while loading the Instructor Dashboard."
    ...
    if len(paid_modes) == 1:
        ...
        log.error(
            u"Course {course_key} has {num_paid_courses} course modes with payment options. Course must only have "
            u"one paid course mode to enable eCommerce options."
            .format(course_key=unicode(course_key), num_paid_courses=len(paid_modes))
        )
...
def _section_e_commerce(course, access, paid_mode, is_white_label):
