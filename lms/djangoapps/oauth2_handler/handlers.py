...
from student.roles import UserBasedRole, GlobalStaff, CourseStaffRole, CourseInstructorRole
    ...
    def _get_courses_with_access_type(self, user, access_type):
        ...
        if not course_ids:
            if GlobalStaff().has_user(user):
                # Global staff have access to all courses, so just return them all.
                course_ids = [unicode(course.id) for course in _get_all_courses()]
            else:
                course_ids = list(UserBasedRole(user, access_type).courses_with_role()
